...
class classical_algorithms:
    ...
    def IC(self):
        largest_cc = max(nx.connected_components(self.G), key=len) #compute the max connected components
        subgraph = self.G.subgraph(largest_cc) #construct the max connected subgraph and compute its information_centrality
